...
public class ProtobufClient {
    ...
    private <T extends Message> T extractResponseData(final Response response, final Class<T> clazz)
    ...
    public List<ReadingBook> addBook(final ReadingBook book) throws ClientException {
        ...
        return converter.toDomain(ReadingBook.class, extractResponseData(response, Books.class).getBookList());
    }

    public List<ReadingBook> showAllBooks() throws ClientException {
        Response response = blockingStub.showLibrary(createRequest(null));
        return Converter.create().toDomain(ReadingBook.class, extractResponseData(response, Books.class).getBookList());
    }
}
